 ...
 */
class BGMIKillLabelImageProcessor(private val bgWeight: Int = 181, override val keyMultiplier: Int) : LabelImageProcessor {
    ...
    private val multiplyPaint = Paint().apply {
        val mode: PorterDuff.Mode = PorterDuff.Mode.DARKEN
    ...
    companion object {
        private val TAG = BGMIKillLabelImageProcessor::class.java.simpleName
    }
    ...
    override suspend fun newLabelImage(labelInfo: TFResult, labelImage: Bitmap, out: Bitmap) {
        ...
        val cachedBoundingBoxes: List<Rect> = if (labelInfo.cachedMlRects != null) gson.fromJson(
            labelInfo.cachedMlRects,
            rectListType
        ) else emptyList<Rect>()

        val boundingBoxes = cachedBoundingBoxes.ifEmpty {
            Log.d("VISION_CALL", "call performed BGMIKILLLABELIMAGE")

            val json = mlKitOCR.performVision(labelImage)
            val formatted = json?.let { MLKitOCR.formatResponse(it) }
            getBoundingBoxes(
                formatted,
                bottomTolerance = (10f / 89 * labelImage.height).roundToInt()
            ).also {
                labelInfo.cachedMlRects = gson.toJson(it)
            }
        }
    ...
    private fun drawImageFromBoundingBoxes(
        ...
        boundingBoxes.forEach { currentRect ->
            ...
            thresholdPaint.apply {
                //Magic numbers!!!! assuming the threshold 150 works best for 201x89 image
                val ratio = 89 / bgWeight.toFloat() * (labelBitmap.width)
                colorFilter =
                    ColorMatrixColorFilter(createThresholdMatrix(((150 / 89f * ratio)).toInt()))
            }
