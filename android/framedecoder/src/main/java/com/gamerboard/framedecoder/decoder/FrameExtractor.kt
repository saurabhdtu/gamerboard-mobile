...
//                    fail("unexpected result from decoder.dequeueOutputBuffer: " + decoderStatus);
                    ...
                    if (doRender) {
                        ...
                        if (frameCount == Int.MAX_VALUE) {
                            _maxFrames = (_fps).toInt() * duration!!.toInt() / 1000
                        } else {
                            _maxFrames = frameCount
                        }
                        if (decodeCount < _maxFrames) {
                            ...
                            if(verbose) Log.d(TAG, "$decodeCount / Max: $_maxFrames")
                        }
                        decodeCount += 1

                        if (verbose) Log.d(TAG, "FrameTestCount ${decodeCount} ${presentationTimeUs}")
                    }
                }
            }
        }
        val totalSavedFrames = if ((_maxFrames < decodeCount)) _maxFrames else decodeCount
